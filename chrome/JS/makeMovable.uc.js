...
(function () {
  ...
  function main(window) {
    if (window.document.documentElement.getAttribute("chromehidden") != "") return; // return if window is not full

    window.UC_API.Windows.waitWindowLoading(window).then(window => {
      initButtons();
      window.gNavToolbox.addEventListener("customizationstarting", prepareCustomize);
  ...
  function initButtons() {
    if (isInUI("movable-extensions-button")) {
      if (!isChild($("movable-extensions-button"), "unified-extensions-button")) {
        $("movable-extensions-button").appendChild($("unified-extensions-button"));
      }
  ...
  function isInUI(id) {
    for (let target of doc.querySelectorAll(".customization-target")) {
      if (isChild(target, id)) { return true; }
    } return false;
  }

  function isChild(parent, id) { return !!parent.querySelector(`#${id}`); }
